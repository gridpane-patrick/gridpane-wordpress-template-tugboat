...
function bosta_create_edit_pickup_form()
{
    ...
    <select required class='data' name='scheduledTimeSlot' >
        <option  <?php esc_attr(selectdCheck($pickupEditData->scheduledTimeSlot, '10:00 to 13:00'));?>   value='10:00 to 13:00'>10:00AM to 01:00PM</option>
        <option <?php esc_attr(selectdCheck($pickupEditData->scheduledTimeSlot, '13:00 to 16:00'));?>value='13:00 to 16:00' >01:00PM to 04:00PM</option>
    </select>
	<?php
	echo "   </td>
                </tr>
...
<td>  ".esc_html($delivery->cod)." LE</td></tr>";
		...
		if ($result->success === false) {
			...
			$pickups                   = $result
				->result->pickups;
			$checkIfActionButtonNeeded = ! ( $_GET['state'] == 'history-pickups' );
           ...
           <table class='pickups-table'>
			...
			for ( $counter = 0; $counter < count( $pickups ); $counter ++ ) {
				...
				$pickupLocationName      = $pickups[ $counter ]->locationName ?: 'N/A';
