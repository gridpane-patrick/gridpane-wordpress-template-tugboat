...
function bosta_override_checkout_state_fields($fields): array {
	...
	for ($i = 0; $i < count($resultStates); $i++) {
        if(isset($resultStates[$i])) {
            for ($x = 0; $x < count($resultStates[$i]); $x++) {
                if (isset($resultStates[$i][$x]->zoneName) && isset($resultStates[$i][$x]->districtName)) {
	...
	$fields['shipping']['shipping_state'] = $state_args;
	$fields['billing']['billing_state'] = $state_args; // Also change for billing field
		...
		if (empty(get_post_meta($order->id, 'bosta_tracking_number', true))) {
			...
			if (is_numeric($order
				...
				$shippingDistrictId = array_merge(...$resultStates)[$order
					...
					->state]->districtId;
				$newOrder
					->$addressType->districtId = $shippingDistrictId;
			} else {
				...
				$shippingCityName = $order
					...
					->city;
				$newOrder
					->$addressType->districtName = $shippingDistrictName;
				$newOrder
					->$addressType->zone = $shippingZoneName;
				$newOrder
...
function bosta_fetch_status_top_bar_button($which)
{
	...
	if ('shop_order' === $typenow && 'edit.php' === $pagenow && isset($_GET['fetch_status']) && $_GET['fetch_status'] === 'yes') {
		...
		bosta_fetch_latest_status_action('fetch_latest_status', $orders);
	}
}

function bosta_fetch_latest_status_action($action, $order_ids)
{
