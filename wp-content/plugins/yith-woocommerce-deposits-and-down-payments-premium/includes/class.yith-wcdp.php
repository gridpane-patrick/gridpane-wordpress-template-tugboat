	...
	class YITH_WCDP {
		...
		public function print_deposit_order_item( $item_id, $item, $order ) {
			if ( isset( $item['deposit'] ) && $item['deposit'] ) {
				...
				$full_amount_html =apply_filters('yith_wcdp_full_amount_order_item_html',wc_price( yith_wcdp_get_price_to_display( $product, array(
					'qty'   => intval( $item['qty'] ),
					'price' => $full_amount,
					'order' => $order
				) ) ), $full_amount, $item,$order );
				...
				echo apply_filters( 'yith_wcdp_print_deposit_order_item_template', $template, $item, $order );
			}
		}
