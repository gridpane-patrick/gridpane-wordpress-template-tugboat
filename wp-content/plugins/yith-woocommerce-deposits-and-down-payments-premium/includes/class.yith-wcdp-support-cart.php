...
if ( ! class_exists( 'YITH_WCDP_Support_Cart' ) ) {
	...
	class YITH_WCDP_Support_Cart extends WC_Cart {
		 ...
		 * @since 1.3.11
		 */
		public function _stash_current_notices() {
			$this->_notices = wc_get_notices();
		}
		 ...
		 * @since 1.3.11
		 */
		public function _restore_notices() {
			wc_clear_notices();

			if ( ! empty( $this->_notices ) ) {
				foreach ( $this->_notices as $notice_type => $notices ) {
