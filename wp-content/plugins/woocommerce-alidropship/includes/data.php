...
class VI_WOOCOMMERCE_ALIDROPSHIP_DATA {
	...
	private static function parse_data_from_AU( $data, $ignore_ship_from, $ignore_ship_from_default ) {
		$result = [];
		...
		if ( ! self::is_currency_supported( $result['currency_code'] ) ) {
			...
			if ( 'RUB' === $result['currency_code'] ) {
				$result['message'] = esc_html__( 'Please configure RUB/USD rate in the plugin settings/Product price', 'woocommerce-alidropship' );
			} else {
				$result['message'] = esc_html__( 'Please switch AliExpress currency to USD', 'woocommerce-alidropship' );
			}
		...
		self::handle_sku_module( $sku_module, $ignore_ship_from, $ignore_ship_from_default, $result );
...
//				'CNY'
	 ...
	 * @param $attributes
	 */
	private static function handle_sku_module( $skuModule, $ignore_ship_from, $ignore_ship_from_default, &$attributes ) {
...
//			'RU'
	 ...
	 * @param int $quantity
	 * @param string $currency Only use USD at the moment
	 * @param string $freight_ext
	 * @param string $province
	 * @param string $city
	 *
...
//			'maxPrice'      => '1',
		...
		$adjust_freight = false;
		$shipping_tax   = self::get_ali_tax( $country );
		if ( 'CZ' === $country && isset( $args['ext'] ) ) {
			/*This applies tax to AliExpress shipping which is required to show saver shipping, super economy... for CZ, not tested with other countries*/
			$args['ext'] = str_replace( '"p6":""', '"p6":"' . number_format( $shipping_tax, 6 ) . '"', $args['ext'], $re_count );
			if ( $re_count ) {
				$adjust_freight = true;
			}
		}
		$url              = add_query_arg( $args, 'https://www.aliexpress.com/aeglodetailweb/api/logistics/freight' );
		$request          = self::wp_remote_get( $url );
		$response['code'] = $request['code'];
		if ( $request['status'] === 'success' ) {
			$data = $request['data'];
			if ( isset( $data['body'] ) && isset( $data['body']['freightResult'] ) && is_array( $data['body']['freightResult'] ) ) {
				$response['status'] = 'success';
				$freight            = $data['body']['freightResult'];
				if ( $adjust_freight ) {
					/*Remove tax from AliExpress shipping if added above*/
					foreach ( $freight as &$freight_ ) {
						if ( ! empty( $freight_['standardFreightAmount']['value'] ) ) {
							$freight_['standardFreightAmount']['value'] = number_format( $freight_['standardFreightAmount']['value'] / ( 1 + $shipping_tax ), 2 );
						}
						if ( ! empty( $freight_['freightAmount']['value'] ) ) {
							$freight_['freightAmount']['value'] = number_format( $freight_['freightAmount']['value'] / ( 1 + $shipping_tax ), 2 );
						}
					}
				}
		...
		if ( $post_id && ! is_wp_error( $post_id ) ) {
			...
			if ( count( $variations ) ) {
				...
				if ( $woocommerce_currency === $currency_code ) {
					...
					'RUB',//allow importing from RUB as aliexpress.ru uses only RUB now
//					'CNY'
				), true ) ) {
