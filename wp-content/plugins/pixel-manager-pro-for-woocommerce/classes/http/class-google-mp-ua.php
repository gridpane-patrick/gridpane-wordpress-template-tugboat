...
class Google_MP_UA extends Google_MP {
	...
	public function __construct( $options ) {
		...
		$this->post_request_args['blocking'] = apply_filters_deprecated('wooptpm_send_http_api_ga_ua_requests_blocking', [$this->post_request_args['blocking']], '1.13.0', 'wpm_send_http_api_ga_ua_requests_blocking');
		$this->post_request_args['blocking'] = apply_filters('wpm_send_http_api_ga_ua_requests_blocking', $this->post_request_args['blocking']);
	}
...
//		error_log(print_r($data_user_identifier, true));
...
//		error_log($this->server_base_path . '?' . http_build_query($payload));
	...
	protected function get_all_order_products( $order ) {
		...
		foreach ($this->wpm_get_order_items($order) as $item_id => $item) {
