...
//			}
	...
	protected function get_v2a_microdata( $product ) {
		...
		$product_dyn_r_ids = $this->get_dyn_r_ids($product);

		$microdata = [
			'@context'    => 'https://schema.org',
			'@type'       => 'Product',
			'productID'   => $product_dyn_r_ids[$this->get_dyn_r_id_type()],
		...
		if ($this->get_brand_name($product->get_id())) {
			$microdata['brand'] = $this->get_brand_name($product->get_id());
		}
		...
		if ($product->is_type('variation')) {
			...
			$parent_product_dyn_r_ids = $this->get_dyn_r_ids($parent_product);

			$microdata['additionalProperty'] = [
				[
					'@type'      => 'PropertyValue',
					'propertyID' => 'item_group_id',
					'value'      => $parent_product_dyn_r_ids[$this->get_dyn_r_id_type()],
				]
			];
		}
