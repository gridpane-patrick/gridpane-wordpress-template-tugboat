	...
	class YITH_WooCommerce_Product_Shipping {
		...
		public function register_plugin_for_activation() {
			if ( ! class_exists( 'YIT_Plugin_Licence' ) ) {
				require_once YITH_WCPS_DIR . '/plugin-fw/lib/yit-plugin-licence.php';
			}
		...
		public function register_plugin_for_updates() {
			if ( ! class_exists( 'YIT_Upgrade' ) ) {
				require_once( YITH_WCPS_DIR . '/plugin-fw/lib/yit-upgrade.php' );
			}
			...
			if ( $method->id == 'yith_wc_product_shipping_method' ) {
				...
				foreach( WC()->cart->get_cart() as $cart_item ) {
					...
					if ( $is_not_sold_individually ) {
						...
						if ( $item_cost > 0 || $shipping_cost > 0 || $product_cost > 0 ) {
				...
				if ( $cart_unique_cost > 0 || apply_filters( 'yith_wcps_cart_unique_cost_free', false ) ) {
