 ...
 * @return boolean
 */
function wps_ubo_org_replace_plugin() {
	$plugin_slug = 'upsell-order-bump-offer-for-woocommerce/upsell-order-bump-offer-for-woocommerce.php';
	$plugin_name = 'Upsell Order Bump Offer for WooCommerce';
	$plugin_zip  = 'https://downloads.wordpress.org/plugin/upsell-order-bump-offer-for-woocommerce.zip';

	ob_start();

	if ( wps_ubo_org_is_plugin_installed( $plugin_slug ) ) {
		wps_ubo_org_upgrade_plugin( $plugin_slug );
		$installed = true;
	} else {
		$installed = wps_ubo_org_install_plugin( $plugin_zip );
	}
 ...
 * @return boolean
 */
function wps_ubo_org_is_plugin_installed( $slug ) {
 ...
 * @return boolean
 */
function wps_ubo_org_install_plugin( $plugin_zip ) {
 ...
 */
function wps_ubo_org_upgrade_plugin( $plugin_slug ) {
	include_once ABSPATH . 'wp-admin/includes/class-wp-upgrader.php';
	wp_cache_flush();
	$upgrader = new Plugin_Upgrader();
