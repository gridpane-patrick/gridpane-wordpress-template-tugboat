...
class WC_Gateway_OPay_RC_Code extends WC_Payment_Gateway {

	public function __construct() {
		$this->id = 'rc_code';
		$this->icon = OPGFW_WC_OPAY_URL . '/img/opay_reference_code.png';
		$this->has_fields = false;
		$this->method_title = 'OPay Reference Code';
		$this->method_description = 'OPay provide merchants with the tools and services needed to accept online payments. Supports consumers to pay with OPay Reference Code.';
		...
		$this->rccodecurrency = $this->get_option('rccodecurrency');
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'orderexpirationtime' => array(
				...
				'type' => 'text',
				'default' => 1440,
				'description' => 'For example, if you enter 1440, the unpaid order will be cancelled automatically after 1440 minutes.',
				'custom_attributes' => array( 'required' => 'required' ),
			),
			'rccodeautocompleteorder' => array(
    ...
    try {
			...
			$logger->add('rc_code', 'Order return result：' . $response);
