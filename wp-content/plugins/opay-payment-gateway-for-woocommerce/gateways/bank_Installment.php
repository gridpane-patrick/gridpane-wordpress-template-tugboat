...
class WC_Gateway_OPay_Bank_Installment extends WC_Payment_Gateway {

	public function __construct() {
		$this->id = 'bank_installment';
		$this->icon = OPGFW_WC_OPAY_URL . '/img/BANK_INSTALLMENT.png';
		$this->has_fields = false;
		$this->method_title = 'Bank Installment';
		$this->method_description = 'OPay provide consumers with convenient Bank Installment payment methods.It is an agreement where the payer authorizes the payment for a single purchase to be split into a number of payments processed at agreed intervals. For example, pay for a purchase in six monthly installments.';
		...
		$this->bankinstallmentcurrency = $this->get_option('bankinstallmentcurrency');
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'bankinstallmentautocompleteorder' => array(
	...
	public function process_payment ( $order_id ) {
		...
		$currency = $this->bankinstallmentcurrency;
		if (!$currency) {
			$currency = 'EGP';
			$country = 'EG';
		} else {
    ...
    $order_str = 'WC-'.date('Ymd',time()).mt_rand(10,99);
      ...
      'reference' => $order_str.'-'.$order_id,
		...
		$logger->add('bank_installment', 'body string：' . $data2);
		$timestamp = time();
		$authString = 'RequestBody=' . $data2 . '&RequestTimestamp=' . $timestamp;
		$auth = $this->auth($authString);
		$logger->add('bank_installment', 'auth：' . $auth);
		$header = [
		...
		try {
			...
			$logger->add('bank_installment', 'Order return result：' . $response);
