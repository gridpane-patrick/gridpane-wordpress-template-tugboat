	...
	class YWPAR_YITH_WooCommerce_Subscription {
		...
		public function assign_points_on_renew( $subscription, $order ) {
			...
			if ( $subscription->fee > 0 ) {
				$conversion    = $parent_order->get_meta( '_ywpar_conversion_points' );
				$point_earned -= $subscription->get_fee() / $conversion['money'] * $conversion['points'];
			}

			if ( $point_earned > 0 ) {
				$customer = ywpar_get_customer( $order->get_customer_id() );
				if ( $customer ) {
					$order->update_meta_data( '_ywpar_points_earned', $point_earned );
					...
					$customer->update_points( $point_earned, 'renew_order', array( 'order_id' => $order->get_id() ) );
					$order->save();
				}
			}
