...
if ( ! class_exists( 'YITH_WC_Points_Rewards_Extra_Points_Birthdate' ) ) {
	...
	class YITH_WC_Points_Rewards_Extra_Points_Birthdate {
		...
		public static function extra_points_birthdate() {
			...
			$user_query = $wpdb->get_col( "SELECT DISTINCT user_id FROM {$wpdb->base_prefix}usermeta WHERE ( meta_key = 'ywces_birthday' OR  meta_key = 'yith_birthday' ) AND MONTH(meta_value) = MONTH(NOW()) AND DAY(meta_value) = DAY(NOW())" ); //phpcs:ignore
